...
### Scope: this governs the application, not the landing page

Everything in this document — the Core Rules above and every token below — applies to the **application UI** (`/app` and its components) only. The **landing page** (`/`) intentionally runs a separate, more editorial visual system: Newsreader serif headlines, IBM Plex Sans/Mono, rounded corners, gradients and shadows where they earn their place, and its own warmer palette (`#0C0908` canvas, `#4FD8A0` accent) — specified in full in `portfolio-risk-ui/design/tokens.json` and `design_handoff_varense_landing/README.md`. This is a deliberate split, not drift: the app is an instrument, the landing page is marketing copy for that instrument, and they're allowed to look like different things. Don't "fix" the landing page into compliance with the rules above.
...
1. **Film grain.** A pre-rendered, tiling PNG noise texture (not a live SVG `feTurbulence` filter, see postmortem below), 64×64px, generated by `generate_grain.py` with `CONTRAST = 1.0` (no artificial contrast stretch, plain uniform `random.randint(0, 255)` distribution). Applied via a shared `::before` pseudo-element to the page canvas, the sidebar, and every card surface, at final locked opacities:
   - `--grain-opacity-canvas: 0.02`
   - `--grain-opacity-surface: 0.012`
   These values were arrived at by pixel-matching against a known-good reference (the original design exploration page) rather than by eye, stdev and mean-adjacent-pixel-difference measured within ~2-7% of the reference on a clean crop. This is texture, not decoration, it is what makes matte dark surfaces feel physical instead of like a flat CSS `background-color`.

   **Postmortem, worth reading before touching this again:** the grain was originally built as a live SVG `feTurbulence` filter. This rendered correctly in isolated testing but was invisible in the real app in production for an unrelated reason: the shared `::before` rule used `z-index: -1` on a host element that never established its own stacking context (`position: relative` with no explicit `z-index`, no `transform`, no `isolation`). The pseudo-element escaped to a shared ancestor stacking context and lost a paint-order tiebreak against the host's own background, rendering completely hidden despite every computed style reporting as correct. **The fix was `z-index: -1` → `z-index: 1`** on the grain `::before` rule, it still paints behind real content because it's a `::before` and precedes the host's content in DOM/paint order, the tied z-index value doesn't cause bleed-through. Separately, the SVG filter was also replaced with a pre-rendered PNG tile, since live `feTurbulence` rendering is known to be inconsistent across browser/GPU combinations, this was a real improvement in its own right, independent of the stacking bug. If grain ever silently disappears again on a real page while still working on an isolated test page, check the host element's stacking context first, don't assume it's an opacity or asset problem.
