...
const TIP    = { background:'var(--card)', border:'1px solid var(--border)', borderRadius:8, fontSize:11 }

// Colour for A and B
const A_COLOR = '#52b788'
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {w === 'A' && <span style={{ fontSize: 10, background: 'rgba(82,183,136,0.15)', color: A_COLOR, padding: '1px 5px', borderRadius: 3, fontFamily: 'sans-serif' }}>WIN</span>}
        {fmtFn(aVal)}
      </div>
      ...
      <div style={{
        ...
        {fmtFn(bVal)}
        {w === 'B' && <span style={{ fontSize: 10, background: 'rgba(110,168,254,0.15)', color: B_COLOR, padding: '1px 5px', borderRadius: 3, fontFamily: 'sans-serif' }}>WIN</span>}
      </div>
    </div>
  )
}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12, height: '100%', overflowY: 'auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 1fr', alignItems: 'center', flexShrink: 0 }}>
        <div style={{ textAlign: 'right' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(82,183,136,0.1)', border: '1px solid rgba(82,183,136,0.3)', borderRadius: 8, padding: '8px 14px' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 10, color: 'var(--text-muted)', fontFamily: 'monospace' }}>{tickersA.join(', ')}</div>
            </div>
          </div>
        </div>
        <div style={{ textAlign: 'center', fontSize: 11, color: 'var(--text-muted)', fontWeight: 600 }}>VS</div>
        <div>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(110,168,254,0.1)', border: '1px solid rgba(110,168,254,0.3)', borderRadius: 8, padding: '8px 14px' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 10, color: 'var(--text-muted)', fontFamily: 'monospace' }}>{tickersB.join(', ')}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      {overallWinner && (
        <div style={{
          ...
          background: `rgba(${overallWinner === 'A' ? '82,183,136' : '110,168,254'},0.08)`,
          border: `1px solid ${overallColor}`,
          borderRadius: 8, fontSize: 12, color: overallColor, fontWeight: 600, flexShrink: 0,
        }}>
          ...
          <div style={{ display: 'flex', gap: 14, fontSize: 10 }}>
            {[[A_COLOR, nameA, dataA.monte_carlo.p50_final], [B_COLOR, nameB, dataB.monte_carlo.p50_final]].map(([c, n, v]) => (
              <span key={n} style={{ display: 'flex', alignItems: 'center', gap: 5, color: 'var(--text-muted)' }}>
                ...
                {n} — <span style={{ color: c, fontFamily: 'monospace', fontWeight: 600 }}>${Math.round(v).toLocaleString()}</span>
              </span>
            ))}
          </div>
        </div>
        <div style={{ height: 180 }}>
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={mcData} margin={{ top: 4, right: 8, bottom: 0, left: 8 }}>
              ...
              <ReferenceLine y={dataA.monte_carlo.portfolio_value} stroke="rgba(255,255,255,0.08)" strokeDasharray="4 4" />
