...
<style>
  ...
  .insight-box.good  { border-color: #10b981; }
  .insight-box.good  .label { color: #10b981; }
  .insight-box.warn  { border-color: #f59e0b; }
  .insight-box.warn  .label { color: #f59e0b; }
  .insight-box.bad   { border-color: #ef4444; }
  .insight-box.bad   .label { color: #ef4444; }
  .learn-card {
    background: #1e293b;
    border-radius: 10px;
    padding: 16px 18px;
  ...
  .learn-card h4 { margin: 0 0 6px; font-size: 15px; color: #f1f5f9; }
  .learn-card p  { margin: 0; font-size: 13px; color: #94a3b8; line-height: 1.6; }
  .learn-card .example {
    margin-top: 8px;
    font-size: 12px;
    background: #0f172a;
...
def analyse_volatility(vol):
    ...
    if vol < 0.15:
        text += "That's quite calm — similar to a steady, diversified fund. Smooth ride."
        tone = "good"
    elif vol < 0.25:
...
with st.sidebar:
    ...
    if weight_mode == "% sliders":
        st.caption("Each slider's max shrinks as others claim their share. The last stock fills whatever is left.")
        ...
        if total_weight != 100:
            st.caption(f"⚠️ Total: {total_weight}% — slide down to give room to the last stock.")
        else:
            st.caption("✓ 100%")
        ...
        weights       = [v / total_dollars for v in dollar_vals] if total_dollars > 0 else [1/n]*n
        ...
        if total_dollars > 0:
            ...
            st.caption("⚠️ Slide at least one ticker above zero.")
    ...
    st.caption(f"\U0001f4c5 {start_date.strftime('%d %b %Y')} \u2192 {end_date.strftime('%d %b %Y')}")
    ...
    st.caption("Fine to leave as defaults — but here's what they mean if you're curious.")
    ...
    with st.expander("⚙️ VaR confidence level (advanced)"):
        st.caption(
...
tab_analyse, tab_learn = st.tabs(["📊 My Analysis", "📖 Learn — what does it all mean?"])
...
with tab_learn:
    ...
    st.caption("No jargon. No assumed knowledge. If you're new to investing, start here.")
...
with tab_analyse:
    if not run:
        st.info("👈 Set up your portfolio in the sidebar and click **Run Analysis** to begin.")
    ...
    with st.spinner("Fetching market data and running analysis..."):
        try:
            ...
            if portfolio_prices.empty:
                st.error("No data returned. Try a different date range or check your tickers.")
                st.stop()
    ...
    if p["n_days"] < 120:
        st.warning(
            f"⚠️ **Short time period warning** — your analysis only covers {p['n_days']} trading days "
    ...
    if ba and ba["beta"] > 1.8:
        st.warning(
            f"⚠️ **High risk warning** — your portfolio has a Beta of {ba['beta']:.2f}, which is very high. "
    ...
    st.success(
        f"✓ Analysis complete — **{p['start']}** to **{p['end']}** "
        f"({p['n_days']} trading days, {p['n_years']} years)"
    )
    ...
    col1.metric("Annual Return",   f"{m['annualised_return']:.1%}",    help=t["annualised_return"])
    col2.metric("Volatility",      f"{m['annualised_volatility']:.1%}", help=t["annualised_volatility"])
    ...
    fig_ret.add_trace(go.Scatter(
        x=port_cumret.index, y=port_cumret.values,
        name="Your Portfolio",
    ...
    if show_benchmark and benchmark_prices is not None:
        ...
        fig_ret.add_trace(go.Scatter(
            x=bench_cumret.index, y=bench_cumret.values,
    ...
    with col_left:
        ...
        fig_dd.add_trace(go.Scatter(
            x=dd["drawdown_series"].index, y=dd["drawdown_series"].values,
            fill="tozeroy", fillcolor="rgba(239,68,68,0.15)",
            line=dict(color="#ef4444", width=1.5),
            hovertemplate="%{x}<br>Drop from peak: %{y:.1%}<extra></extra>",
        ))
        fig_dd.update_layout(yaxis_tickformat=".0%", margin=dict(l=0, r=0, t=10, b=0), height=280)
    ...
    with col_right:
        st.markdown("### How bumpy the ride was")
        st.caption(
        ...
        fig_vol.add_trace(go.Scatter(
            x=r["rolling_volatility"].index, y=r["rolling_volatility"].values,
            line=dict(color="#f59e0b", width=2), fill="tozeroy",
            fillcolor="rgba(245,158,11,0.1)",
            hovertemplate="%{x}<br>Risk level: %{y:.1%}<extra></extra>",
        ))
        fig_vol.update_layout(yaxis_tickformat=".0%", margin=dict(l=0, r=0, t=10, b=0), height=280)
    ...
    fig_ef.add_trace(go.Scatter(
        x=f["vols"], y=f["returns"], mode="markers",
        marker=dict(
            color=f["sharpes"], colorscale="Viridis",
            size=4, opacity=0.6,
    ...
    fig_ef.add_trace(go.Scatter(
        x=[f["max_sharpe_vol"]], y=[f["max_sharpe_return"]],
        mode="markers+text",
        marker=dict(symbol="star", size=18, color="#f59e0b", line=dict(color="white", width=1)),
        text=["★ Best mix"], textposition="top right",
    ...
    fig_ef.add_trace(go.Scatter(
        x=[f["min_vol_vol"]], y=[f["min_vol_return"]],
        mode="markers+text",
        marker=dict(symbol="diamond", size=14, color="#10b981", line=dict(color="white", width=1)),
        text=["◆ Safest mix"], textposition="top right",
        name="Safest mix",
        hovertemplate=(
    ...
    fig_ef.add_trace(go.Scatter(
        x=[m["annualised_volatility"]], y=[m["annualised_return"]],
        mode="markers+text",
        marker=dict(symbol="circle", size=14, color="#2563eb", line=dict(color="white", width=1)),
    ...
    text, tone = analyse_frontier(
        m["sharpe_ratio"], f["max_sharpe_sharpe"],
        f["max_sharpe_weights"], tickers,
    )
    insight("Efficient Frontier", text, tone)

    with st.expander("★ See the suggested optimal weights"):
        c1, c2 = st.columns(2)
        with c1:
    ...
    with col_left:
        st.markdown("### Do your stocks move together?")
        st.caption(
        ...
        fig_corr = go.Figure(go.Heatmap(
            ...
            text=corr.round(2).values, texttemplate="%{text}", textfont=dict(size=13),
    ...
    with col_right:
        st.markdown("### Your current split")
        st.caption("How your money is currently divided across your stocks.")
        fig_pie = go.Figure(go.Pie(
            labels=tickers, values=[w * 100 for w in weights],
    ...
    with st.expander("📋 See the raw price data"):
        st.caption(f"Closing prices used in this analysis. Showing last 20 of {len(portfolio_prices)} trading days.")
        st.dataframe(portfolio_prices.tail(20).style.format("{:.2f}"), use_container_width=True)
